...
void ofApp::setup(){
    ...
    gui.setup();
    gui.add(instruction.setup("press any key", "un/hide menu"));
    gui.add(frameRate.setup("frame rate", ""));
    gui.add(comparisonThreshold.setup("comparison threshold", 8, 1, 15));
    gui.add(comparisonDistance.setup("comparison distance", 3, 1, 15));
    gui.add(lineWidth.setup("line width", 10, 1, 15));
    gui.add(randomMul.setup("random multiplier", 10, 0, 20));
}
